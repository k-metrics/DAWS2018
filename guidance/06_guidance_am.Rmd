...
　  
`purrr`パッケージはBase Rの`apply`関数群や`plyr`パッケージなどを再設計したようなパッケージで、階層的な処理や反復処理に威力を発揮します。`purrr`パッケージは`tidyverse`ファミリーであり、`tidyverse`パッケージを読み込むと自動的に読み込まれます。  
`purrr`パッケージの中心とも言えるのが`purrr::map`関数群で、この関数群は引数（ベクトルやリスト）に対して指定した処理（関数や演算子）を適用する関数型プログラミング手法を用いたものです。
　  
