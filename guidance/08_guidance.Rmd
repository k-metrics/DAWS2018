...
## コンソール出力のリダイレクト
RやRStudioのコンソール出力をファイルへリダイレクトしたい場合は`sink`関数を使ってください。ただし、R Markdownのチャンク内では有効に機能しません。  
...
なお、コンソール入力（`>`で入力するコマンド）は出力されませんので、コンソール入力も合わせて出力したい場合は`TeachingDemos`パッケージを利用してください。
...
## XPath
![相対パス（構成変更に弱い）と絶対パス（実装が面倒？）](./fig/xpath.png)
