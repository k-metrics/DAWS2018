...
## Gitのインストール
今回はGitHubのリポジトリを利用しますので**Gitが必要**です（Gitクライアントではありません。また、GitHubアカウントも不要です）。Windowsの場合は[Git for Windows](https://gitforwindows.org/)をダウンロードしてインストールしてください。他のOSは公式リポジトリからインストールしてください。
