...
* 前述の入力が完了したら［Create Project］ボタンをクリックしてください
    ...
    * "Packrat mode on. Using library in directory:"と表示されれば完了です
        * 完了メッセージ後もバックグラウンドで色々やるのはWindows環境特有の現象のようなので**しばらく放置**し終わらない場合は強制終了させて再起動します
...
* 何かのエラーで止まったっぽい
    1. コンソールから**packrat::restore()**を実行してみてみます
...
## 動作確認
### ブランチの作成
プロジェクトがインポートされたらGitでブランチを作成します。［Git］タブの［New Branch］をクリックしてダイアログが表示されたら**リモートを「(None)」**にしブランチ名に「feature/test」を入力、［Create］ボタンをクリックします。
