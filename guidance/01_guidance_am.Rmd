...
* 必要なパッケージのダウンロードとインストールが始まります
    * 環境にもよりますが**15〜30分前後の時間**がかかります
        * 気になるならシステムモニタを開いておいてください
        * 特にコンパイルが必要な場合には時間がかかります
    * "Packrat mode on. Using library in directory:"と表示されれば完了です
        * Windows環境では完了メッセージが出た後もバックグラウンドでRのセッションが動き続けることがあります。しばらく放置しても終わらない場合は強制終了させ再起動させてください
...
* 何かのエラーで止まった、パッケージが不足しているかも
    1. コンソールから**packrat::restore()**を実行します（止まった場合）
    1. コンソールから**packrat::status()**を実行します（不足の確認）
