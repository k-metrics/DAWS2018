...
---------|-----------------|-------|--------|-----
R本体    | Yes             | Yes   | Yes    | 3.4.4 or later
RStudio  | Yes             | Yes   | Yes    | Desktop 1.1.423 or later
Rtools34 | Yes, [R3.3.x and later <i class="fa fa-external-link"></i>](http://cran.us.r-project.org/bin/windows/Rtools/){target="_blank" title="Building R for Windows"}             | **No** | **No** | Windows Only
Git      | Yes, [Git for Windows <i class="fa fa-external-link"></i>](https://gitforwindows.org/){target="_blank" title=""} | Yes   | Yes    | Gitクライアントではありません
packart  | Yes             | Yes   | Yes    | CRAN

　  
この手の勉強会としては本邦初？の試みとして**GitHubリポジトリ**と**packrat**パッケージを用いた統一環境の作成を行います。なお、**GitHubアカウントの取得は不要**です。


## Rのインストールと動作確認
環境を統一するためにRのバージョンは統一したいと考えています（R 3.4.4 or R.3.5.x）。追加パッケージは`packrat`を用いることでバージョンを統一します。  
...
### Rtoolsのインストール
[Rtoolsのインストール方法](https://k-metrics.github.io/cabinet/program/env_install_tls.html)を参考にインストールを行ってください。必ず**デフォルトのインストールパス**にインストールしてください。Windows環境以外の方はインストールの必要はありません。
...
## 利用するバージョン
環境を統一するためにRStudioのバージョンは統一（v1.1.423 or later）したいと思います。
...
## プロジェクトのクローンと動作確認
今回は前出の通りGitHubの公開リポジトリと**packrat**パッケージを利用して統一環境の準備を自動的に行うことを試みます。環境によっては上手く動かない可能性もありますが、ご了承ください。なお、**GitHubアカウントでのログインは不要**です。
...
## 動作確認
### ブランチの作成
プロジェクトがインポートされたらGitのブランチを作成します。［Git］タブの［New Branch］をクリックしてダイアログが表示されたら**リモートを「(None)」**にしブランチ名に「feature/test」を入力、［Create］ボタンをクリックします。

![](./fig/RStudio_git.png)

　  
右上の「master」の表示が「feature/test」に変わっていることを確認してください。

## 動作確認
### Rmdファイルのコンパイル
ブランチが作成されたらR Markdownファイルをレンダリングします。
...
　  
以上で環境設定は終了です。`packrat`を利用するプロジェクトはプロジェクトを開いた際に`packrat`環境のチェックが自動的に走りますので開くのに時間がかかります。
