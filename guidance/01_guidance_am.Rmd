...
### **Rtoolsのインストール**
[Rtoolsのインストール方法](../../program/env_install_tls.html)を参考にインストールを行ってください。必ずデフォルトのインストールパスにインストールしてください。Windows環境以外の方はインストールの必要はありません。
...
* **`knitr`**, **`rmarkdown`**, **`tidyverse`**, **`packrat`**
...
1. 以下のURLを［Repository URL］に入力します
    * https://github.com/k-metrics/2018.git
1. ［Project directory name］は"Packrat"と変更してください
1. ［Create project as subdirectory of］は任意ですが、日本語や空白が含まれないパスにしてください
1. 上記の入力が完了したら［Create Project］ボタンをクリックしてください
    * GitHubのリポジトリがクローニングされてRStudioでプロジェクトが開かれます
    * YAMLに関するエラーが出ますが無視してください
    * "No Source tarball at packrat available localy"と表示され必要なパッケージが見当たらない場合はインストールされますのでしばらくお待ちください
    * "Installing R package forim source requires install of additional build tools."と出ますがRtoolsがデフォルトパスにインストール済の場合は［No］ボタンをクリックしてください

1. プロジェクトをクローズし、再度、プロジェクト（"Packrat"）を開きます
