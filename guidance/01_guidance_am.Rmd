...
---------|-----------------|-------|--------|-----
R本体    | Yes, [download](https://cran.ism.ac.jp/banner.shtml)             | Yes, [download](https://cran.ism.ac.jp/banner.shtml)   | Yes, [download](https://cran.ism.ac.jp/banner.shtml)    | 3.4.4
RStudio  | Yes, [download](https://www.rstudio.com/products/rstudio/download/#download)             | Yes, [download](https://www.rstudio.com/products/rstudio/download/#download)   | Yes, [download](https://www.rstudio.com/products/rstudio/download/#download)    | Desktop 1.1.442 or later
Rtools34 | Yes, [download](http://cran.us.r-project.org/bin/windows/Rtools/)             | **No** | **No** | Windows Only
Git      | Yes, [download](https://gitforwindows.org/) | Yes   | Yes    | Gitクライアントではありません
packart, devtools  | Yes             | Yes   | Yes    | CRAN

　  
この手の勉強会としては本邦初？の試みとして**GitHubリポジトリ**と**packrat**パッケージを用いた環境の統一を行います。なお、**GitHubアカウントの取得は不要**です。
...
### Rtoolsのインストール
[Rtoolsのインストール方法](https://k-metrics.github.io/cabinet/program/env_install_tls.html)を参考にインストールを行ってください。必ず**デフォルト・オプション**でインストールしてください。Windows環境以外の方はインストールの必要はありません。
...
## リカバリタイム
* `packrat::restore()`を実行したけどプロンプトが表示されない
    * コンソールの右上に赤いアイコンが出ている場合は動作中です
* `packrat::status()`を実行したけどプロンプトが表示されない
    * コンソールの右上に赤いアイコンが出ている場合は動作中です

## 動作確認
### ブランチの作成
プロジェクトが作成できたら作業用のブランチを作成します。［Git］タブの［New Branch］をクリックしてダイアログが表示されたら**リモートを「(None)」**にしブランチ名に「feature/test」を入力し［Create］ボタンをクリックします。
