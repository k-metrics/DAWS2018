...
## モデル式の評価
（線形）重回帰モデルを評価するには線形単回帰モデルと同様に決定係数とp値に着目する。  
...
```
　  
決定係数（adj.r.squared）は説明変数が目的変数の変動の何パーセントを説明できているかを意味する値でこの場合は約$82\%$を説明変数で説明できていることが分かる。一方、モデルp値（p.value）は回帰式自体の有意性を表している。


## モデル式の判定（つづき）
モデル式自体の評価を行ったら次に回帰係数のp値（p.value）を評価する。  
...
```
　  
この場合、切片と`cyl`、`drat`、`wt`を除いて適しているとは言い難い。


## 変数選択 
このような場合、AIC値（赤池情報量基準）を元にしたステップワイズ変数選択を用いて最も適していると考えらえる変量を選択する方法がある。
```{r, include=FALSE}
result <- step(lm(mpg ~ cyl + disp + hp + drat + wt + qsec + vs + am + gear + carb,
     data = mtcars)) %>% broom::tidy()
```

```{r, eval=FALSE}
step(lm(mpg ~ cyl + disp + hp + drat + wt + qsec + vs + am + gear + carb,
     data = mtcars)) %>% broom::tidy()
```
...
このモデル式では決定係数が$0.8335$であり、先程のモデルより多くの変動を説明できていることが分かる。
...
## 分散説明率
分散説明率とは分散の総和に対する各主成分得点の分散（の合計）の比。  
<!-- $$\frac{主成分得点の分散（の合計）}{分散の総和（各変量の分散の総和=主成分得点の分散の総和）}$$ -->
...
mtcars %>% dplyr::select(wt, qsec, am) %>% 
  ...
  dplyr::mutate(sum = sum(value)) %>% 
  tidyr::spread(key, value) %>% 
  dplyr::mutate(PC1 = round(`1`/sum, 2), PC2 = round(`2`/sum, 2),
                PC3 = round(`3`/sum, 2)) %>% 
