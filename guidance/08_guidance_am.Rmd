...
* 指標を使用して主成分数を決める
    1. 累積**分散説明率**（7〜8割以上ならまあいいかな）
    1. 値が「1」を超える**固有値の数**
    1. 降順に並べた固有値の折れ線の「**屈曲点** - 1」
...
## 主成分数を決める（分散説明率）
主成分数を決めるためにまず分散説明率を確認する。分散説明率とは分散の総和に対する各主成分得点の分散の比のこと。
...
## 主成分数を決める（固有値）
固有値は各変量（変数）の**主成分負荷量**の二乗和。主成分負荷量は**主成分得点と変量（変数）との相関係数**。固有値が1を超えているものは主成分として採用できる。  
...
主成分負荷量が「$0$」（または、$0$に近い値）になれば、対応する主成分は、その変量に対する影響がない（少ない）ということであり、より、合理的な説明がしやすくなる。  
