...
* **データから分析結果が再現できる**こと
    ...
    * [CRAN Taskの一つ（Reproducible Research）<i class="fa fa-external-link"></i>](https://cran.r-project.org/web/views/ReproducibleResearch.html){target="_blank" title="Reproducible Research"}
...
* R Markdownを使うと
    ...
    * **リバイズ（改訂）対応**が容易
        ...
        * Gitのような版管理システム（VCS）と組み合わせることで変更点の把握が容易
...
* データ分析はソフトウェア開発者と同じようなプログラミング**周辺知識を必要**とする時代
    * BTS(Redmine, etc), VSC(Git, etc), DBMS(SQL), VM(Docker, etc), ...
* 分析技術も必要だけど**使えるデータ**（$\neq$記録）を作らせるスキルも必要
