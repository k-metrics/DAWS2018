...
演算子種別     | 演算子（優先順）
---------------|-----
アクセス演算子 | `$`（テータフレーム型の列参照）, `[]`（ベクトル型の値参照）, `.`（引数参照：パイプ演算子内）
単項演算子     | `-`（マイナス）
算術演算子     | `*`（乗算）, `/`（除算）, `+`（加算）, `-`（減算）
比較演算子     | `<`, `>`, `<=`, `>=`, `==`, `!=`
論理演算子     | `!`（否定）, `&`（論理積）, `|`（論理和）
...
## 代入演算子（`<-`）
代入演算子はその名の通り演算結果などを代入します。代入方向は`<-`の通り演算子の右辺を演算子の左辺に代入します。パイプ演算子が連続する場合は最終結果が代入されます。
