...
## 変数の型
変数には以下の型があり型宣言は不要です（代入すれば自動的にデータに見合った型の変数が作成されます）。主に使うのはベクトル型とデータフレーム型です。  

変数型           | 説明
-----------------|-----
ベクトル型       | **一種類のデータ型**の値を**任意の個数**扱える変数型
マトリクス型（行列型） | ベクトル型の一形態（特殊なベクトル型とも言える）
アレイ型（配列型）     | 同上
データフレーム型 | **異なるデータ型で同じ長さ**のベクトル変数を複数個まとめた変数型
リスト型         | データフレーム型より柔軟性のある変数型
因子型           | 特殊な変数構造をもつベクトル型変数


## データ型
変数の値として取りうるデータの型には以下のものがあります。
...
演算子種別     | 演算子（優先順）
---------------|-----
アクセス演算子 | `$`（テータフレーム型の列への）, `[]`（ベクトル型の値への）
単項演算子     | `-`（マイナス）
算術演算子     | `*`（乗算）, `/`（除算）, `+`（加算）, `-`（減算）
比較演算子     | `<`, `>`, `<=`, `>=`, `==`, `!=`
論理演算子     | `!`（否定）, `&`（論理積）, `|`（論理和）
...
```{r}
iris_mean <- iris %>% dplyr::group_by(Species) %>%
