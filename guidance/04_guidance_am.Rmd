...
<center>![The Datasaurus Doze](../data/DinoSequential.gif)</center>
...
## irisとは？
アイリス（ギリシャ語のイリスが語源）とはアヤメ科アヤメ属の多年草。花は三個の花弁（内花、Petal）と三個の萼片（外花、Sepal）から構成される。Setosaの和名はヒオウギアヤメ、他は不明。iris datasetはフィッシャーの線形判別分析に用いられたデータらしい（[irisの正体](http://d.hatena.ne.jp/tsutatsutatsuta/20121206/1354737461)）。
...
データセットを指定したら各座標に割り当てるデータやグルーピングするためのデータなどを指定します。どのようなグラフでも指定方法はggplot2の文法にしたがいます。なので、エステティック関数（aes）の使い方が二つ目のポイントになります。（[最新版チートシート](https://github.com/rstudio/cheatsheets/blob/master/data-visualization-2.1.pdf)）
