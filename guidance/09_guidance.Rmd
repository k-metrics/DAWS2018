...
## 対象データ
以下のデータは[セリーグの個人投手成績（最終結果）](http://npb.jp/bis/2018/stats/pit_c.html)です。次ページ以降の主成分分析結果を見て成績上位の投手がどのような特徴を持っているか分析してください。  

```{r, include=FALSE, eval=FALSE}
df <- "http://npb.jp/bis/2018/stats/pit_c.html" %>% 
  xml2::read_html() %>% 
...
# names(df) <- "http://npb.jp/bis/2018/stats/pit_c.html" %>% 
#   xml2::read_html() %>% 
#   rvest::html_node(xpath = "/html/body/div/div/div/div[3]/table[1]") %>% 
#   rvest::html_table() %>% 
#   dplyr::slice(2)
...
header <- "./data/cpitch_2018.csv" %>% 
  readr::read_csv(locale = locale(encoding = "UTF-8")) %>% 
  dplyr::select(row = `順位`, `投手名`, `チーム`, `カテゴリ`)

"./data/cpitch_2018.csv" %>% 
  readr::read_csv(locale = locale(encoding = "UTF-8")) %>% 
  dplyr::select(-`順位`, -`投手名`, -`チーム`, -`カテゴリ`, -`ホール`) %>% 
  prcomp(., scale. = TRUE) %>% 
  broom::tidy() %>% 
  tidyr::spread(key = PC, value = value) %>% 
