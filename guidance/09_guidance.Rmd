...
## 変量（変数）間の関係
予測や制御を行う前には変量間の関係を把握しておくことが大切です。変量間の関係は以下のように分類できます。 [出典 <i class="fa fa-external-link"></i>](){target="_blank" title=""}  
...
* 目的変数の値を大きく（または小さく）するには説明変数（これから与える値）をどうかする（**制御**）【因果関係がある場合】
...
## 主成分分析とは
主成分分析は前述の次元縮約を使って任意数の変量（変数）から「共通因子」を探索し「可視化」できる分析方法です。乱暴にいえばｎ次元空間の変量（変数）の関係を２次元空間の主成分の関係へ写像する方法です。  
