...
```
原因はエラーメッセージに表示されている通り重複行があるため、どこに展開すべきか判断がつかないためです。  
　  
`part`と`value`をひとまとめにして表示すると分かりやすいですが、この場合は7行目と8行目がワイド型にする場合に何行目へ配置すべきか区別がつかないことが分かります。
```{r}
iris %>% 
  dplyr::filter(Petal.Width == 0.1) %>% 
  tidyr::gather(key = "part", value = "value", -Species) %>% 
...
　  
これを解決するには、予め行識別ができるようなIDを付与しておきます。
```{r}
iris %>% 
  dplyr::filter(Petal.Width == 0.1) %>% 
...
　  
先程の7行目と8行目は識別番号による区別出来るようになっていることが分かります。
```{r}
iris %>% 
  dplyr::filter(Petal.Width == 0.1) %>% 
