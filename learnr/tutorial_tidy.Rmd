...
file_path <- "C:\\Users\\suzuki\\Documents\\R\\2018dasg\\learnr/issues.csv"
bugs <- readr::read_csv(file_path, locale = readr::locale(encoding = "UTF8"), progress = FALSE)
```
...
### 必要なデータ
前述のように具体的な事例としてBTSのデータを用います。BTSのデータは[Redmine.JP](http://redmine.jp/glossary/i/issue/)で公開されている[サンプルDB](http://www.redmine.org/projects/redmine/issues)を用います。GitHubのリポジトリに配置しますのでネットワークが接続できる環境が必要です。ご自身でデータを用意する場合は以下の手順で作成、配置してください。
...
## Tidy/Overview
次に読み込んだデータを扱いやすいように整形します。BTSは通常、項目がカスタマイズされていて様々なデータが登録されていますが、ここでは以下の項目を対象としますので必要な項目のみを抽出（選択）します。なおWindows環境では項目に日本語があるとチュートリアル動作に影響があるため項目名を全てascii文字に変更してあります。
...
bugs %>% 
  dplyr::select(id, tracker, status, open) %>% 
  dplyr::rename(ID = id, Tracker = tracer, Status = status,
                DateTime = open) %>% 
  dplyr::mutate(Date = lubridate::as_date(DateTime)) %>% 
```
