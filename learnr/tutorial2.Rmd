...
```{r quantile_fivenum, exercise=TRUE, exercise.lines=3}

```

```{r quantile_fivenum-solution}
...
## Chapter 8 - hist
### ヒストグラム
ヒストグラムは度数分布表を元に描かれますがRでは関数一つで描くことが可能です。`iris`データセットの四つの数値データに対するヒストグラムを`hist`関数を用いて描いてみましょう。
```{r, eval=FALSE}
...
```{r lm_model, exercise=TRUE, exercise.lines=5}
...
平均値50、標準偏差10の正規分布の確率密度分布を描いてみましょう。描く範囲は任意としますが、なるべく分布の形が分かるような範囲として下さい。コメントを外して適切な引数を指定して実行してください。
