...
# setup chunk内で設定したオブジェクトはどのexercise chunkからも参照可能
bugs <- "https://k-metrics.github.io/2018dasg/learnr/issues.csv" %>%
  read_csv(locale = locale(encoding = "UTF-8"), progress = FALSE)

odc_data <- "https://k-metrics.github.io/2018dasg/learnr/odc_e.csv" %>%
  read_csv(locale = locale(encoding = "CP932"), progress = FALSE)
```
...
odc_data %>% 
  ...
  #               Dtype = '欠陥タイプ') %>% 
  dplyr::select(Status = status, Result = solve, Date = found,
                Rank = rank, Step = process,
                Trigger = trigger, Type = reason,
                Dtype = defect_type) %>% 
...
このようにタグを用いて効率よくバグの原因や傾向を把握できるODB分析はいかに効率よく集計、可視化できるかもポイントのひとつだと思います。集計、可視化を[R Markdown][RM]で実装しておけば、定期的なレポートは[R Markdown][RM]をknitするだけで作成できるというメリットがあります。
...
tickets <- bugs %>% 
  ...
  dplyr::select(ID =id, Tracker = tracker, Status = status,
...
bugs %>% 
  ...
  #               Category = 'カテゴリ', 
  #               OpenDateTime = '作成日', CloseDateTime = '終了日') %>% 
  dplyr::select(ID =id, Tracker = tracker, Status = status, Category = category,
                OpenDateTime = open, CloseDateTime = close) %>% 
  dplyr::filter(Tracker == "Defect" & Status != "Closed") %>% 
